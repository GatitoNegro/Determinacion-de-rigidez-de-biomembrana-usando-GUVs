...
def binarizar(imagen,n1,n2):
  median = cv2.medianBlur(imagen,n1) #filtro de la mediana con kernel 
  binarizada = cv2.adaptiveThreshold (median, 255 , cv2.ADAPTIVE_THRESH_GAUSSIAN_C , cv2.THRESH_BINARY,5,0) #ingresar ultimos 2 parametros
...
def calcularcontorno(binarizadam):
    ...
    #if len (contorno)==0:
    #  st.wite ('cambiar parametros de filtrado, no hay contornos en algunos frames')
    j = 0
...
with header:
    ...
    if uploader is not None:
        ...
        with col5:
          imagen_binarizada = binarizar(video_np[x], n1=kernel1, n2=kernel2)
          st.image(imagen_binarizada, caption=f'Frame numero {x} binarizado', clamp=True)
        with col6:
          contorno = calcularcontorno(imagen_binarizada)
        ...
        for j in range(frames):
          ...
          imagen_binarizada = binarizar(imagen, kernel1, kernel2)
          contorno = calcularcontorno(imagen_binarizada)
          dibujocontorno = dibujarcontorno(contorno,0) #por que esta aca esta?
